...
function deployAzureInfrastructure(){
    ...
    cmd="az deployment group create \
        ...
        --template-file $SCRIPTS_DIRECTORY/arm-template.user-assigned.json \
        --output none \
        --parameters \
    ...
    USER_IDENTITY_CLIENT_ID=$(az deployment group show --resource-group $resourcegroup -n $datadep | jq -r '.properties.outputs.identityClientId.value')
    # get User Assigned Identity Principal ID
    USER_IDENTITY_PRINCIPAL_ID=$(az deployment group show --resource-group $resourcegroup -n $datadep | jq -r '.properties.outputs.identityPrincipalId.value')
    # get User Assigned Identity Name
    USER_IDENTITY_NAME=$(az deployment group show --resource-group $resourcegroup -n $datadep | jq -r '.properties.outputs.identityName.value')
}
...
printMessage "Azure User Assigned Identity Name: ${USER_IDENTITY_NAME}"
printMessage "Azure User Assigned Identity Client Id: ${USER_IDENTITY_CLIENT_ID}"
printMessage "Azure User Assigned Identity Principal Id: ${USER_IDENTITY_PRINCIPAL_ID}"
...
deployWebAppContainer "$AZURE_SUBSCRIPTION_ID" "$AZURE_APP_PREFIX" "${ACR_LOGIN_SERVER}" "${ACR_NAME}" "${USER_IDENTITY_NAME}" "dotnet-rest-api" "latest" "${APP_VERSION}" "${APP_PORT}"
